...
void R_GenerateLookup (int texnum)
{
	...
	{
	    char error[256];
            sprintf(error, "R_GenerateLookup: column without a patch (%s)\n", texture->name);
            I_Error(error);
	    return;
	}
	...
	if (patchcount[x] > 1)
	{
	    ...
	    {
                char error[256];
		sprintf(error, "R_GenerateLookup: texture %i is >64k", texnum);
                I_Error(error);
	    }
...
//r_data.c:451: error: unused variable 'temp3'
//r_data.c:450: error: unused variable 'temp2'
//r_data.c:449: error: unused variable 'temp1'
//    int			temp1;
//    int			temp2;
//    int			temp3;
...
//        printf("%s\n", texture->name);
	...
	for (j=0 ; j<texture->patchcount ; j++, mpatch++, patch++)
	{
	    ...
	    {
                char error[256];
		sprintf(error, "R_InitTextures: Missing patch in texture %s", texture->name);
                I_Error(error);
	    }
	}
...
int	R_TextureNumForName (char* name)
{
    ...
    {
        char error[256];
	sprintf(error, "R_TextureNumForName: %s not found", name);
	I_Error(error);
    }
    return i;
}
